...
export const dbHelpers = {
  ...
  // Update user profile
  async updateUserProfile(userId: string, updates: Partial<TablesUpdate<'user_profiles'>>) {
    const { data, error } = await (supabaseAdmin
      .from('user_profiles'))
      .update({
        ...updates,
...
export const realtimeHelpers = {
  ...
  subscribeToConversation(conversationId: string, callback: (payload: { [key: string]: unknown }) => void) {
    return supabase
  ...
  subscribeToUserNotifications(userId: string, callback: (payload: { [key: string]: unknown }) => void) {
    return supabase
      .channel(`user:${userId}`)
      .on(
        'postgres_changes',
