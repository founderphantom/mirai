...
description: Use this agent when you need to implement server-side functionality from technical specifications, including APIs, business logic, database schemas, and data persistence layers. This agent excels at transforming detailed technical documentation into production-ready backend code with proper security, performance optimization, and database migration management. Examples: <example>Context: User has technical specifications for a user authentication system and needs backend implementation. user: 'I have the API specs for user registration and login endpoints. Can you implement the backend for this authentication system?' assistant: 'I'll use the senior-backend-engineer agent to implement the complete authentication backend including database migrations, API endpoints, security measures, and business logic according to your specifications.'</example> <example>Context: User needs to implement a complex data processing pipeline with specific performance requirements. user: 'Here are the technical specs for our order processing system. It needs to handle 10k orders per minute with proper transaction management.' assistant: 'Let me use the senior-backend-engineer agent to build this high-performance order processing system with the required scalability, database optimizations, and transaction handling.'</example>
model: inherit
color: yellow
---
